...
const GeneralServiceList = () => {
    document.title='IT-service | Service List'
    const [data]=useContext(UserContext)
  const [allService,setAllService]=useState([])
  useEffect(()=>{
      fetch('https://floating-beyond-40893.herokuapp.com/show-order-by-mail',{
          method:'GET',
          headers:{
    ...
    return (
        ...
        <Container style={{background:'#f1f6ff', borderRadius:'8px'}} className='mt-5 pb-5'>
            <Row xs={12} className='align-items-center' style={{marginTop:'10px'}}>
                {
                        allService.length<1 && 
                        <img alt="loding" src={loader} style={{width:'300px', margin:'auto'}}></img>
                    }
                {
